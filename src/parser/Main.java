...
/**
 *	FOR CONVERTING BITE FILES TO ARFF FILES
 */
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for(File file : bitesFolder.listFiles()) {
			...
			BufferedWriter bw =
					new BufferedWriter(new FileWriter("C:\\temp\\bite_counting\\data.arff", true));
		...
		for (File file : nonBitesFolder.listFiles()) {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter("C:\\temp\\bite_counting\\data.arff", true));
			for (int k = 0; k < xAxis.size(); k++) {
				bw.write(xAxis.get(k) + "," + yAxis.get(k) + "," + zAxis.get(k) + "," + 
						 xStandardDev + "," + yStandardDev + "," + zStandardDev + "," +
						 xJerk.get(k) + "," + yJerk.get(k) + "," + zJerk.get(k) + "," +
